...
def get_dataloaders(dataset, idx, bs=None, fast=True, verbose=True):
    ...
    trainset = Dload(dataset, batch_size=bs[0], sampler=train_sample, drop_last=True)
    valset = Dload(dataset, batch_size=bs[1], sampler=val_sample, drop_last=True)
    testset = Dload(dataset, batch_size=bs[2], sampler=test_sample, drop_last=True)
