...
def train_epoch_nn(net, dataloader, optimizer, loss_function, binary, mlp_only=False):
    ...
    for batch in dataloader:
        ...
        #if mlp_only : output = net(X.view(-1, X.shape[1]))
        if mlp_only : output = net(X)
        else : output = net.forward_encoder(X.view(-1, X.shape[1]))
...
def eval_epoch(net, dataloader, loss_function, binary, mlp_only=False):
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            if binary:
                y = y.unsqueeze(1)
            if mlp_only : output = net(X)
            else : output = net(X.view(-1, X.shape[1]))
            loss = loss_function(output, y)
            epoch_loss += loss.item()
            eval_steps += 1
...
def train_nn(
    ...
    for epoch in range(config["epochs"]):
        train_loss, train_steps = train_epoch_nn(
            net, trainset, optimizer, loss_function, binary, mlp_only
        )
        if val:
            val_loss, val_steps, val_y_pred, val_y_true = eval_epoch(
                net, valset, loss_function, binary, mlp_only
            )
        if test:
            test_loss, test_steps, test_y_pred, test_y_true = eval_epoch(
                net, testset, loss_function, binary, mlp_only
            )
